...
class AutoSign(object):
    ...
    def __init__(self, name,username, password, schoolid=None, enc=None):
        """初始化就进行登录"""
        self.headers = {
    ...
    async def get_all_classid(self) -> list:
        ...
        res = []
        async with self.session.request(method='GET',
                                        url='http://mooc1-2.chaoxing.com/visit/courselistdata?courseType=1&courseFolderId=0&courseFolderSize=1') as resp:
            text = await resp.text()
